...
public class App {
    ...
    private static void removeNumber() {
        ...
        System.out.println("Informe a coluna em que o número ");
        var col = runUntilGetValidNumber(0, 8);

        System.out.println("Informe a linha em que o número ");
        var row = runUntilGetValidNumber(0, 8);
        System.out.printf("Informe o número que você deseja limpar na posição [%s,%s]\n", col, row);
        if (!board.limparEspaco(col, row)) {
            System.out.printf("A posição [%s, %s] tem um valor fixo\n", col, row);
        }
    }
    ...
    private static void showCurrentGame() {
        ...
        for (int i = 0; i < BOARD_LIMIT; i++) {
            for(var col : board.getEspacos()){
                args[argPos++] = String.format("%2s", col.get(i).getAtual() != null ? col.get(i).getAtual() : "");
            }
        }
    ...
    private static void finishGame() {
       if (isNull(board)) {
        ...
        if (board.jogoAcabou()) {
            ...
            System.out.println("Você ainda prencher algum espaço");
        }
    }
